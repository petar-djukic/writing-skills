...
4. **CoT Leakage Detection** (Prompt 4) — Find reasoning scaffolding that regex missed, including bridge sentences at paragraph boundaries. For each candidate, Prompt 4 applies the removal test: delete the sentence, check whether the paragraph loses information. True leaks are flagged for deletion; CoT-style wording on real content is flagged for rewording; brief echoes are flagged for recast. Its types are numbered by their **catalog** number, and it carries the categories the semantic pass earns — the absent ones are absent on purpose, each recorded at its catalog entry under **Detection ownership** (Category 8 is Prompt 6's; 10, 11 and 12 are closed lexical forms the regex owns). `test_cot_coverage.py` fails if a category is added without that decision being made (GH-40).
