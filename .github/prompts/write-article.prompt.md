---
description: "Complete a Substack article from a brainstorm file or outline. Research supporting data, verify sources, and produce a publication-ready article follo"
---

Execute the /write-article command. The full workflow follows; treat any
...
```bash
python3 .github/skills/match-structure/scripts/voice_anchors.py anchors \
...
Before drafting, read `.github/skills/filter-tells/references/drafting-guidance.md`.
...
```bash
python3 .github/skills/match-structure/scripts/voice_anchors.py profile --for "$ART"
```
...
- **Pass `--stratum pre-ai`, then check what retrieval actually chose.** Read
  ...
  ```bash
  python3 .github/skills/match-voice/scripts/drive.py --article "$ART" \
  ...
  **The register problem is anchor concentration, not role.** Earlier guidance
  ...
  ```bash
  python3 .github/skills/match-voice/scripts/retrieve.py \
...
```bash
python3 .github/skills/tighten-style/scripts/tighten.py \
