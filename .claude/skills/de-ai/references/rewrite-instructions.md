...
| Parallelism | Restructure so neither the heads nor the tails mirror. Use different syntactic patterns (questions, conditional, relative clause, participial phrase), then check **both ends** of the pair before accepting: a fix that only changes the openings pushes the parallelism to the tails ("…whatever model you run." / "…whatever you run it on."), which the surface scripts do not catch. Break the sentence at a different point, not just the front. |
