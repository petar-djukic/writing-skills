...
```bash
python3 .agents/skills/match-structure/scripts/voice_anchors.py anchors \
...
Before drafting, read `.agents/skills/filter-tells/references/drafting-guidance.md`.
...
```bash
python3 .agents/skills/match-structure/scripts/voice_anchors.py profile --for "$ART"
```
...
- **Pass `--stratum pre-ai`, then check what retrieval actually chose.** Read
  ...
  ```bash
  python3 .agents/skills/match-voice/scripts/drive.py --article "$ART" \
  ...
  **The register problem is anchor concentration, not role.** Earlier guidance
  ...
  ```bash
  python3 .agents/skills/match-voice/scripts/retrieve.py \
...
```bash
python3 .agents/skills/tighten-style/scripts/tighten.py \
